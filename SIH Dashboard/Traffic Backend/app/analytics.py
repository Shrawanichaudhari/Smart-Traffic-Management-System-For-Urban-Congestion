...
class AnalyticsService:
    ...
    async def get_environmental_impact(
        ...
        with get_db_context() as db:
            ...
            if comparison_mode and len(data) >= 2:
                ai_data = [d for d in data if d['mode'] == 'ai']
                baseline_data = [d for d in data if d['mode'] == 'baseline']
                
                if ai_data and baseline_data:
                    ai_avg_fuel = sum(d['avg_fuel_saved_per_cycle'] for d in ai_data) / len(ai_data)
                    baseline_avg_fuel = sum(d['avg_fuel_saved_per_cycle'] for d in baseline_data) / len(baseline_data)
                    
                    fuel_improvement = ai_avg_fuel - baseline_avg_fuel
                    fuel_improvement_pct = (fuel_improvement / baseline_avg_fuel * 100) if baseline_avg_fuel > 0 else 0
                    
                    return {
                        "environmental_data": data,
                        "comparison": {
                            "fuel_improvement_liters": round(fuel_improvement, 3),
                            "fuel_improvement_percentage": round(fuel_improvement_pct, 2),
                            "co2_reduction_kg": round((ai_avg_fuel - baseline_avg_fuel) * 2.3, 3)
                        }
                    }
    ...
    async def get_baseline_vs_ai_comparison(
        ...
        with get_db_context() as db:
            ...
            # Group by mode
            results = query.group_by(Simulation.mode).all()
            ...
            if 'ai' in comparison_data and 'baseline' in comparison_data:
                ...
                # Calculate percentage improvements (negative means AI is better for wait_time)
